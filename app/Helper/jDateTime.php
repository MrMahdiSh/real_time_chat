...
class jDateTime{
    ...
    public static function explodeDate($datetime)
    {
        $ex = explode(' ', $datetime);
        return $ex[0];
    }
    public static function explodeTime($datetime)
    {
        $ex = explode(' ', $datetime);
        return $ex[1];
    }
    ...
    public static function mktimeUnivers( $year , $month , $day , $hour , $minute , $second , $jalali = null, $timezone = null)
    {
        //Defaults
        $month = (intval($month) == 0) ? self::date('m') : $month;
        $day   = (intval($day)   == 0) ? self::date('d') : $day;
    ...
    public static function datetimeToArray($datetime, $dateDelimiter="-"){
        $splitTimeStamp = explode(' ',$datetime);
        $date = $splitTimeStamp[0];
        $time = isset($splitTimeStamp[1]) ? $splitTimeStamp[1] : "00:00:00";//date('H:i:s',strtotime($value));
    ...
    public static function toJalaliStr($datetime,$format="Y/m/d H:i:s"){
        $date = new self(false, true, 'Asia/Tehran');
        $time = $date->mktime($datetime['hour'],$datetime['minute'],$datetime['second'],$datetime['month'],$datetime['day'],$datetime['year'],false);
        return $date->date($format, $time);
    }

    public static function toGregorianStr($datetime,$format="Y-m-d H:i:s"){
        //$date = new self(false, true, 'Asia/Tehran');
        $date = new self(false, true, 'GMT');
        $time = $date->mktime($datetime['hour'],$datetime['minute'],$datetime['second'],$datetime['month'],$datetime['day'],$datetime['year'] );
        //$time = $date->mktimeUnivers($datetime['year'],$datetime['month'],$datetime['day'],$datetime['hour'],$datetime['minute'],$datetime['second']);
        return $date->gDate($format, $time);
    }

    public static function isValidJalaliDate($date, $delimiter="/")
    {
        ...
        if (count($dateArray) == 3){
            ...
            if(!self::isLeapYear($year) and $month == 12 and $day == 30){
                return false;
            }
            return true;
        }else{
