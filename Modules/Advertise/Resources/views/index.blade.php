...
@section('content')
    ...
    <div class="app-content content">
        ...
        <div class="content-wrapper">
            ...
            @include('partials.breadcrumb',['title'=>'لیست   تبلیغات'])
                            ...
                            @if(isset($data))
                                ...
                                @foreach($data as $key=>$item)
                                    <tr>
                                        ...
                                        <td class="product-name">{{$item->size==\Modules\Advertise\Entities\BannerSize::FullSize ? 'تمام صفحه'  : 'نیم صفحه'}}</td>
                                        <td class="product-name">{{$item->title}}</td>
                                        <td class="product-name">{{$item->link}}</td>
                                        <td class="product-action">
                                            @if(auth()->user()->can('Advertise.edit'))
                                                <a href="{{route('AdvertiseIndex.edit',['AdvertiseIndex'=>$item->id])}}"><span
                                                        class="action-edit"><i class="feather icon-edit"></i></span></a>
                                            @endif
                                            @if(auth()->user()->can('Advertise.destroy'))
                                                <a href="#"
                                                   onclick="delete_item('{{route('AdvertiseIndex.destroy',['AdvertiseIndex'=>$item->id])}}')">
                                                    <span class=""><i
