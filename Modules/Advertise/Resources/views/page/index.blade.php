...
@section('content')
    ...
    <div class="app-content content">
        ...
        <div class="content-wrapper">
            ...
            @include('partials.breadcrumb',['title'=>'لیست صفحات تبلیغات'])
                        ...
                        <table class="table data-list-view">
                            ...
                            @if(isset($data))
                                ...
                                @foreach($data as $key=>$item)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        @if($item->type==\Modules\Advertise\Entities\BannerType::Media)
                                        ...
                                        <td class="product-name">{{$item->route_name}}</td>
                                        <td class="product-action">
                                            @if(auth()->user()->can('Advertise.edit'))
                                                <a href="{{route('AdvertisePage.edit',['AdvertisePage'=>$item->id])}}"><span
                                                        class="action-edit"><i class="feather icon-edit"></i></span></a>
                                            @endif
                                            @if(auth()->user()->can('Advertise.destroy'))
                                                <a href="#"
                                                   onclick="delete_item('{{route('AdvertisePage.destroy',['AdvertisePage'=>$item->id])}}')">
                                                    <span class=""><i
