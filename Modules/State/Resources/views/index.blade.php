...
@section('content')
    ...
    <div class="app-content content">
        ...
        <div class="content-wrapper">
            ...
            @include('partials.breadcrumb',['title'=>'لیست استان'])
                            ...
                            @if(isset($data))
                                ...
                                @foreach($data as $key=>$item)
                                    <tr>
                                        ...
                                        <td class="product-name">{{$item->cities()->count()}}</td>

                                        <td class="product-action">

                                            @if(auth()->user()->can('State.create'))
                                                <a href="{{route('State.show',['State'=>$item->id])}}"><span
                                                        title="ثبت شهر" class="action-edit"><i
                                                            class="feather icon-circle"></i></span></a>
                                            @endif


                                            @if(auth()->user()->can('State.edit'))
                                                <a href="{{route('State.edit',['State'=>$item->id])}}"><span
                                                        class="action-edit"><i class="feather icon-edit"></i></span></a>
                                            @endif
                                            @if(auth()->user()->can('State.destroy'))
                                                <a href="#"
                                                   onclick="delete_item('{{route('State.destroy',['State'=>$item->id])}}')">
                                                    <span class=""><i
