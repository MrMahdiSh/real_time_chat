...
@section('content')
    ...
    <div class="app-content content">
        ...
        <div class="content-wrapper">
            ...
            @include('partials.breadcrumb',['title'=>'لیست تگ ها'])
                                ...
                                @foreach($data as $key=>$item)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{route('ArticleTag.status',['id'=>$item->id])}}">

                                                <div
                                                    class="chip {{(int)$item->status==1 ? 'chip-success' : 'chip-danger'}}">
                                                    <div class="chip-body">
                                                        <div
                                                            class="chip-text">(جهت تغییر_کلیک شود){{(int)$item->status==1 ? 'فعال' : 'غیرفعال'}}</div>
                                                    </div>
                                                </div>
                                            </a>
                                        ...
                                        <td class="product-action">
                                            @if(auth()->user()->can('Article.edit'))
                                                <a href="{{route('ArticleTag.edit',['ArticleTag'=>$item->id])}}"><span
                                                        class="action-edit"><i class="feather icon-edit"></i></span></a>
                                            @endif
                                            @if(auth()->user()->can('Article.destroy'))
                                                <a href="#"
                                                   onclick="delete_item('{{route('ArticleTag.destroy',['ArticleTag'=>$item->id])}}')">
                                                    <span class=""><i
