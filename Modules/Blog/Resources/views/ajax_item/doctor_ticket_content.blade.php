    ...
    <div class="chat-header">
        ...
        <div class="media d-flex">
            <div class="media-img-wrap flex-shrink-0">
                <div class="avatar  ">
                    <img
                        src="{{isset($data[0]->media) ? $data[0]->media->url : asset('assets/img/avatar.jpg')}}"
                        alt="User Image"
                        class="avatar-img rounded-circle">
                </div>
            </div>
            <div class="media-body flex-grow-1">
                <div class="user-name">{{$data[0]->full_name}}</div>
        ...
        <ul class="list-unstyled">
            ...
            @isset($data)
                @foreach($data as $item)
                    ...
                    <li class="media d-flex  {{(int)$item->sender_id==(int)$item->user_id ? 'received' : 'sent'}}">


                        @if((int)$item->sender_id==(int)$item->user_id)
                            <div class="avatar flex-shrink-0">
                                <img
                                    src="{{isset($data[0]->user->media) ? $data[0]->user->media->url : asset('assets/img/avatar.jpg')}}"
                                    alt="User Image"
                                    class="avatar-img rounded-circle">
                            </div>
                        @endif
