            ...
            <div class="doc-info-left">
                <div class="doctor-img">
                    <a href="{{route('doctor_profile',['doctor'=>$item->full_name,'id'=>$item->id])}}">
                        <img  loading="lazy"    src="{{isset($item->media) ? $item->media->url : asset('assets/img/avatar.jpg')}}"
                ...
                <div class="doc-info-cont">
                    ...
                    @if($item->category)
                        <h5 class="doc-department"><img
                                src="{{$item->category->media ? $item->category->media->url : ''}}" class="img-fluid"
                                alt="Speciality"> {{$item->category ? $item->category->title : ''}}</h5>
                                        ...
                                        <li>
                                            <a target="_blank" href="{{$val->url}}"
                                               data-fancybox="gallery">
                                                <img src="{{$val->url}}" alt="{{$item->clinic->title}}">
                                            </a>
                                        </li>
                ...
                <div class="clinic-booking">
                    <a class="view-pro-btn"
                       href="{{route('doctor_profile',['doctor'=>$item->full_name,'id'=>$item->id])}}">
                        مشاهده پروفایل</a>
                    <a class="apt-btn"
                       href="{{route('doctor_service_reserved',['service_id'=>$service_id,'doctor_id'=>$item->id])}}">درخواست
                        نوبت
                    </a>
                </div>
            </div>
